...
const SCORING_SYSTEM_PROMPT = `You are an expert evaluator who scores the quality of LLM outputs. Score the output based on the following criteria:
...
export async function calculateGptScore(
  variant: string,
  output: string,
...
Please evaluate the output and provide scores.`
    ...
    return calculateHeuristicScore(output);
  } catch (error) {
